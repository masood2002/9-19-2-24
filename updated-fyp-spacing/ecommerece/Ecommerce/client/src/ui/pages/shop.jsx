...
function Shop() {
  ...
  const initialCategory = queryParams.get('category'); // Get the initial category from the URL
  ...
  useEffect(() => {
    if (initialCategory) {
      setChecked([initialCategory]); // Set the initial category as checked
    }
  }, [initialCategory]);
                ...
                <VStack h={'fit-content'} mt={3} justify={"flex-start"} align={"flex-start"} spacing={'1px'} >
                  {categories.map((c) => (
                    <Checkbox
                      ...
                      onChange={(e) => handleCategoryFilter(e.target.checked, c._id)}
                      colorScheme="red"
                      isChecked={checked.includes(c._id)} // Mark the checkbox as checked if it's selected
                    >
                      <Text mt={"12px"} fontWeight='400'>{c.name}</Text>
                    </Checkbox>
                  ))}
